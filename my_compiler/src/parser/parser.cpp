...
static std::runtime_error synerr(const Token& t, const char* msg) {
    std::ostringstream os; os << "line " << t.line << ": col " << t.col << ": " << msg; return std::runtime_error(os.str());
}
...
void Parser::expect(TokenKind k, const char* msg) { if (!accept(k)) throw synerr(peek(), msg); }
...
Type Parser::typeSpec() {
    ...
    switch (t.kind) {
        ...
        default: throw synerr(t, "type expected");
    }
...
Type Parser::afterTypeModifiers(Type base) {
    ...
    while (accept(TokenKind::LBracket)) {
        Token d = eat();
        if (d.kind != TokenKind::Integer) throw synerr(d, "array size integer expected");
...
Param Parser::param() {
    Type t = typeSpec();
    Token id = eat();
    if (id.kind != TokenKind::Identifier) throw synerr(id, "param name expected");
    parseArraySuffix(t);
    return {t, id.text};
}
...
std::unique_ptr<Stmt> Parser::declOrExprStmt() {
    ...
    if (peek().kind==TokenKind::KwInt || peek().kind==TokenKind::KwChar || peek().kind==TokenKind::KwFloat || peek().kind==TokenKind::KwVoid) {
        Type t = typeSpec();
        Token id = eat(); if (id.kind!=TokenKind::Identifier) throw synerr(id, "identifier expected");
        parseArraySuffix(t);
...
std::unique_ptr<Expr> Parser::primary() {
    ...
    switch (t.kind) {
        ...
        default: throw synerr(t, "expression expected");
    }
}

std::unique_ptr<Function> Parser::function() {
    Type ret = typeSpec();
    Token id = eat(); if (id.kind!=TokenKind::Identifier) throw synerr(id, "function name");
    expect(TokenKind::LParen, "(");
